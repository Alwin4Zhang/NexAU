...
2. **阶段 ②**(AG-UI events 缺口补齐):引入 `LLMCallMetadataEvent` 等 sidecar event,让 AG-UI 端能承载 ModelResponse 所有字段(`stop_reason` / `model_name` 等)
...
**结论**:nexau 早已在 ag_ui 之上做扩展。本 RFC 加新的 sidecar event 类型(如 `LLMCallMetadataEvent`)是**既有模式的延续**,不破坏惯例。
...
- **新增 `LLMCallMetadataEvent`**(对照 `UsageUpdateEvent` 既有先例)在 LLM 调用结束时 emit,承载 `stop_reason` / `model_name` / `id` / 任何 provider-specific metadata。
...
```python
class LLMCallMetadataEvent(BaseEvent):
    """LLM call 完成时 emit 一次,承载 ModelResponse 需要但 ag_ui 标准 events 没的字段。"""

    type: Literal["LLM_CALL_METADATA"] = "LLM_CALL_METADATA"
    run_id: str
...
- `LLMCallMetadataEvent` 必须**晚于**所有 message-level event,**早于** `RunFinishedEvent`
- `finalize()` 只能调一次
...
1. 在 `nexau/archs/llm/llm_aggregators/events.py` 新增 `LLMCallMetadataEvent`、扩展 `ThinkingTextMessageStart/EndEvent` 字段
...
4. `agent_events_middleware.after_model()` 不再从 `model_response` 读 usage,改为订阅 `LLMCallMetadataEvent`(去掉对 Set B 的隐式依赖)
...
4. **把 `stop_reason` / `model_name` 塞到 `MessageEndEvent.extra` 而不新增 `LLMCallMetadataEvent`**:被否——`stop_reason` 是整次 LLM call 的属性,不属于任何单条 message;塞 MessageEndEvent 语义混乱。
...
- [ ] **阶段 ② — AG-UI events 缺口补齐(PR-B)**
  ...
  - `agent_events_middleware` 改为订阅 `LLMCallMetadataEvent`(去 Set B 依赖)
...
- `nexau/archs/llm/llm_aggregators/events.py` — 新增 `LLMCallMetadataEvent`(阶段 ②)
...
2. **阶段 ② 中 `agent_events_middleware.after_model` 的过渡期行为**:在阶段 ② merge 但阶段 ③ 未完成时,middleware 是否应该同时支持订阅新 `LLMCallMetadataEvent` 和读 `model_response.usage` 作为 fallback?需要决定。
