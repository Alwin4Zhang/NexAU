...
3. 不在第一阶段实现:plugin pack (`extends`)、activation events、跨 plugin 引用 (`${config.<other>.*}`)、MCP discovered tool 的 per-call 参数注入、partial load、远程 `use:` scheme、`contributes.system_prompt_fragment` (plugin 直接拼 system prompt 的能力,业务上下文统一走 skill 通道);
...
Plugin 只在加载期存在,展开后转为现有 `mcp_servers / middlewares / skills / sub_agents / tools`,runtime 不感知 plugin。
...
> 注:本 RFC Phase 1 **不引入** `contributes.system_prompt_fragment`(plugin 不能直接拼 system prompt)。plugin 想注入业务上下文 / persona / 使用约束的需求,统一走 `contributes.skills[*]` (skill brief 进 registry 即可被 model 感知,detail 通过 `LoadSkill` 按需加载)。理由:跟 NexAU 现有"按需加载"哲学一致,避免每个对话都付 always-on prompt token;且 plugin 直接改 system prompt 会把 plugin scope 从"加能力"扩到"改 agent 行为本身"。后续 RFC 若验证 skill 通道不足以覆盖某类刚需,再考虑加回。
...
contributes:
  ...
  tools:                                  # 字段集 = ToolConfigEntry
    - name: query_order
      yaml_path: tools/query_order/query_order.tool.yaml
...
| `tools[]` | `ToolConfigEntry` (name / yaml_path / binding / lazy / as_skill / defer_loading / extra_kwargs) | `yaml_path` 指向 plugin 内 tool YAML |
| `skills[]` | `{ name, path }` | path 须含 `SKILL.md`,plugin 根目录内 |
| `sub_agents[]` | `SubAgentConfigEntry` (name / config_path) | config_path 须在 plugin 根目录内 |
...
2. **plugin manifest 侧**:adapter 校验得到 `resolved config` 后,渲染 manifest 内的 `${config.<name>}` 和 `${plugin.dir}`。
...
| plugin manifest 渲染 | `${plugin.dir}` | `use:` resolver 得到的 plugin 包绝对根目录 | 当前 plugin 的 `plugin.yaml` contributions 及 plugin-contributed sub-agent YAML inline 渲染 | 用于引用 plugin 内脚本 / 文件;adapter 仍需做路径越界检查 |
...
`contributes.sub_agents[*].config_path` 指向的 YAML 是 plugin 内部文件。adapter 展开时把已校验的 plugin `resolved config` 注入解析上下文,**inline 渲染**所有 `${config.<key>}`,再喂给 `AgentConfig.from_yaml()`。**不依赖** `SubAgentConfigEntry` schema 变更。
...
| `system_prompt` / `system_prompt_suffix` / `llm_config` / `sandbox_config` / `before_*_hooks` / `after_*_hooks` / `tracers` / `token_counter` | plugin 不能 contribute | — |
...
- `PluginAdapter` 正确展开 `contributes.{mcp_servers, tools, skills, sub_agents, middlewares}` 为现有 schema;
